...
def thesis():
    ...
    return render_template("thesis.html", comments=comments)
...
def add_resources():
    ...
    if request.method == "POST":
        ...
        if up_file:
            ...
            up_file.save(
                os.path.join(
                    app.config["UPLOAD_FOLDER"], secure_filename(up_file.filename)
                )
            )
    all_resource = Resources.query.all()
    return render_template(
        "add_resources.html", all_resource=all_resource, filename2=filename2
    )
...
def get_file(sno):
    ...
    return Response(
        resource.up_file, content_type="application/octet-stream"
    )  # Update the content type based on the file type
...
def comment():
    return render_template(
        "comment.html", course_data=zip(course_codes, Serials, comments, ratings)
    )
...
def alumni():
    return render_template(
        "alumni.html", alumnyy=zip(names, emails, c_names, c_types, dess)
    )
...
def intern():
    return render_template(
        "intern.html", internn=zip(i_names, positions, requiremments, i_infos)
    )
